...
function localSnapshot()
{
	if (ocx)
	{
		var snapshotFormat = $("#snapshotformat").val();
		var snapshotPath = $("#snapshotpath").val();
		if (snapshotPath == "")
		{
			snapshotPath = "C:/IVS/LocalSnapshotPath";
            $("#snapshotPath").val(snapshotPath);
		}
		
		// var localCaptureConfigxml = "";
		// localCaptureConfigxml += "<CaptureConfig>";
		// localCaptureConfigxml += "    <CapturePath>" + snapshotPath + "</CapturePath>";                // 本地抓拍保存路径  
		// localCaptureConfigxml += "    <CaptureDownloadPath>" + snapshotPath + "</CaptureDownloadPath>";// 本地抓拍保存路径
		// localCaptureConfigxml += "    <SnapshotMode>0</SnapshotMode>";                                 // 抓拍模式："0"为单张抓拍，"1"为连续抓拍
		// localCaptureConfigxml += "    <SnapshotCount>1</SnapshotCount>";                               // 本地抓拍数量
		// localCaptureConfigxml += "    <SnapshotInterval>1</SnapshotInterval>";                         // 本地抓拍间隔
		// localCaptureConfigxml += "    <SnapshotFormat>" + snapshotFormat + "</SnapshotFormat>";        // 本地抓拍图片格式
		// localCaptureConfigxml += "    <NameRule>1</NameRule>";                                         // 图片文件命名规则
		// localCaptureConfigxml += "</CaptureConfig>";
		
		// var result = ocx.IVS_OCX_SetLocalCaptureConfig(localCaptureConfigxml);
		// if (result != 0)
		// {
			// $("#resultcode").val("IVS_OCX_SetLocalCaptureConfig:" +result);
			// alert(langs[lang]["setSnapshotParamError"]);
			// return;
		// }
        ...
        result = ocx.IVS_OCX_LocalSnapshotToDir(wnd, 1, snapshotPath);
...
function startDownload(downloadType)
{
	if (ocx)
	{
        ...
        if(downloadfilePath == "")
        {
            downloadFilePath = "C:/IVS/RecordDownloadFile";
            $("#downloadFilePath").val(downloadFilePath);
        }
