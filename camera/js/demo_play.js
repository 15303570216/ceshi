	...
	if (ocx)
	{
		...
		if (result == 0)
		{
			...
			for (i = 0; i < recordDataInfoNode.length; i++)
			{
				...
				var nvrCode = xmlDoc.find("RecordDataInfo:eq(" + i + ")").find("NVRCode").text();
				 
				var htmlStr = "<div>" + 
						"<input type='text' id='cameraCode" + i + "' value='"+ cameraCode+ "' style='width: 120px;' readonly='readonly'>" +
						"<span>|</span>" +  
						"<input type='text' id='NVRCode" + i + "' value='"+ nvrCode+ "' style='width: 120px;' readonly='readonly'>" +
						"<span>|</span>" +  
 ...
 * @param id 窗格编号
 */
function startPlayBack(s,e,wId,r,c,n)
{
	console.log(s,e,wId,r,c,n);
	if (ocx) 
	{
		var wnd = wId;
		var startTime = s;
		...
		playbackParam += "        <NVRCode>" + nvrCode + "</NVRCode>";         // NVR编码
		playbackParam += "    </PlaybackParam>";
		playbackParam += "</Content>";
 ...
 */
function getRecordList1(f,t,r,c)
{
	if (ocx)
	{
		var fromTime = f;
